...
## Задача 1.19 (z-Перетворення)

Правосторонній сигнал f[·] з z-перетворенням

F(z) = 1/(1 + 1/2z⁻¹) + 1/(1 - 2z⁻¹)

має бути передавальною функцією ЛІС-системи (Лінійної Інваріантної в часі Системи).

a) Чи є ця система каузальною?

б) Чи є ця система ВХВС (вхід-обмежений вихід-стійкою)?

в) Обчисліть f[0], використовуючи властивість початкового значення z-перетворення.

г) Визначте керовану канонічну форму та використайте її для обчислення перших трьох членів (f[0], f[1] та f[2]) імпульсної характеристики.

Для розв'язання цієї задачі потрібно:

1) Для пункту а):
   - Система є каузальною, якщо її передавальна функція не має додатних степенів z

2) Для пункту б):
   - Система є ВХВС стійкою, якщо всі полюси передавальної функції лежать всередині одиничного кола

3) Для пункту в):
   - Використати формулу початкового значення:
   f[0] = lim(z→∞) F(z)

4) Для пункту г):
   - Привести передавальну функцію до керованої канонічної форми
   - Знайти перші три члени імпульсної характеристики системи через розклад в ряд

Давайте розв'яжемо кожен пункт:

1) а) Перевірка каузальності:
   F(z) = 1/(1 + 1/2z⁻¹) + 1/(1 - 2z⁻¹)
   = z/(z + 1/2) + z/(z - 2)
   = (z² + 3/2z)/(z² - 3/2z + 1)

   Оскільки степінь чисельника не перевищує степінь знаменника і всі члени містять невід'ємні степені z, система є каузальною.

2) б) Перевірка ВХВС стійкості:
   Знайдемо полюси передавальної функції:
   z² - 3/2z + 1 = 0
   z = (3/2 ± √(9/4 - 4))/2
   z = (3/2 ± √(-7/4))/2

   Полюси є комплексними числами з модулем |z| = 1, тому система знаходиться на межі стійкості і не є ВХВС стійкою.

3) в) Обчислення f[0]:
   f[0] = lim(z→∞) F(z)
   = lim(z→∞) [(z² + 3/2z)/(z² - 3/2z + 1)]
   = 1

4) г) Керована канонічна форма:
   F(z) = (z² + 3/2z)/(z² - 3/2z + 1)
   = 1 + (5/2z⁻¹ + 2z⁻² + ...)
...
Кінцева відповідь:
а) Так, система є каузальною.
б) Ні, система не є ВХВС стійкою.
в) f[0] = 1
г) Перші три члени імпульсної характеристики:
   f[0] = 1
   f[1] = 5/2
   f[2] = 2
...
Python-код для розв'язання кожного з пунктів цієї задачі, який включає розрахунки каузальності, стійкості, обчислення \( f[0] \), а також знаходження перших трьох значень імпульсної характеристики системи:
...
# Задаємо функцію F(z)
F_z = 1 / (1 + 1 / (2 * z)) + 1 / (1 - 2 / z)

# Спрощуємо F(z)
F_z_simplified = sp.simplify(F_z)
print("Передавальна функція F(z):", F_z_simplified)

# Пункт а) Перевірка каузальності
# Функція є каузальною, якщо її члени не мають додатних степенів z
is_causal = all(exp <= 0 for term, exp in F_z_simplified.as_poly(z).as_dict().items())
print("Система є каузальною:", is_causal)

# Пункт б) Перевірка ВХВС стійкості
# Полюси передавальної функції
denominator = sp.denom(F_z_simplified)
poles = sp.solve(denominator, z)

# Визначення, чи всі полюси знаходяться всередині одиничного кола
is_stable = all(abs(pole) < 1 for pole in poles if pole.is_real)
print("Система є ВХВС стійкою:", is_stable)

# Пункт в) Обчислення f[0] за допомогою властивості початкового значення
# f[0] = lim(z → ∞) F(z)
f_0 = sp.limit(F_z_simplified, z, sp.oo)
print("f[0] =", f_0)

# Пункт г) Обчислення перших трьох членів імпульсної характеристики через розкладання в ряд Лорана
# Знаходимо коефіцієнти f[0], f[1], f[2]
F_z_series = F_z_simplified.series(z, sp.oo, n=3).removeO()
print("Розклад у ряд Лорана F(z):", F_z_series)

# Коефіцієнти f[0], f[1], f[2]
f_coeffs = [F_z_series.coeff(z, i) for i in range(3)]
print("Перші три члени імпульсної характеристики:")
print("f[0] =", f_coeffs[0])
print("f[1] =", f_coeffs[1])
print("f[2] =", f_coeffs[2])
```

### Пояснення коду:

1. **Передавальна функція \( F(z) \)**: ми спрощуємо її, щоб легше працювати з нею.
   
2. **Перевірка каузальності (Пункт а)**: Аналізуємо, чи передавальна функція містить лише від'ємні степені \( z \), що підтверджує каузальність.

3. **Перевірка ВХВС стійкості (Пункт б)**: Знаходимо полюси функції та перевіряємо, чи всі вони знаходяться всередині одиничного кола (тобто, \( |z| < 1 \)).

4. **Обчислення \( f[0] \) (Пункт в)**: Використовуємо властивість початкового значення \( \lim_{z \to \infty} F(z) \) для обчислення \( f[0] \).

5. **Розклад у ряд Лорана для імпульсної характеристики (Пункт г)**: Розкладаємо \( F(z) \) в ряд Лорана для знаходження коефіцієнтів \( f[0] \), \( f[1] \) і \( f[2] \).
