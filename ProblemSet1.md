...
1) ℓ¹ - це простір сумовних сигналів, тобто сигналів, для яких сума модулів їх значень є скінченною:
   ℓ¹ = {x[n] | Σ|x[n]| < ∞}

2) ℓ² - це простір квадратично сумовних сигналів, тобто сигналів, для яких сума квадратів їх значень є скінченною: 
   ℓ² = {x[n] | Σ|x[n]|² < ∞}

3) Доведення:
   а) ℓ¹ ⊂ ℓ²:
      Нехай x[n] ∈ ℓ¹. Тоді Σ|x[n]| < ∞.
      За нерівністю Коші-Буняковського, (Σ|x[n]|)² ≤ Σ|x[n]|² * Σ1 = Σ|x[n]|².
      Отже, Σ|x[n]|² < ∞, тобто x[n] ∈ ℓ².
      Таким чином, ℓ¹ ⊂ ℓ².

   б) 1/2 ⊂ ℓ²:
      Нехай x[n] = 1/√n, тоді Σ(1/√n)² = Σ1/n = ∞.
      Отже, 1/√n ∉ ℓ².
      Натомість, Σ|1/√n| = Σ1/√n < ∞.
      Тобто, 1/√n ∈ ℓ¹, а значить 1/2 ⊂ ℓ².

Таким чином, доведено, що ℓ¹ ⊂ ℓ² і 1/2 ⊂ ℓ².



### Задача 1.6 (Обмежені сигнали)
Дано лінійний фільтр з передавальною функцією H(z) = 1 + 2z⁻¹ - 3z⁻² + 2z⁻⁴. Нехай x[·] буде вхідним сигналом, а y[·] = (x * h)[·] - вихідним сигналом. Вхідний сигнал задовольняє умову |x[k]| ≤ 1 для всіх k ∈ Z. Визначте найбільше можливе значення |y[k]|, таке, що ця максимальна величина досягається при k = 0.

Для розв'язання цієї задачі потрібно:
1) Обчислити вихідний сигнал y[k] = (x * h)[k] за допомогою згортки вхідного сигналу x[k] з імпульсною характеристикою фільтра h[k].
2) Знайти максимальне значення |y[k]| з урахуванням обмеження |x[k]| ≤ 1.

Почнемо з обчислення згортки:
y[k] = (x * h)[k] = Σ x[n]h[k-n]

Підставляючи передавальну функцію фільтра H(z) = 1 + 2z⁻¹ - 3z⁻² + 2z⁻⁴, отримаємо:
h[k] = 1, k = 0
h[k] = 2, k = 1 
h[k] = -3, k = 2
h[k] = 2, k = 4
h[k] = 0, в інших випадках

Тоді згортка буде:
y[0] = x[0]
y[1] = x[0]*1 + x[1]*2 = 2x[1]
y[2] = x[0]*(-3) + x[1]*1 + x[2]*(-3) = -3x[0] - 3x[2]
y[3] = x[1]*2 = 2x[1] 
y[4] = x[0]*2 + x[4]*2 = 2x[0] + 2x[4]
y[k] = 0 для k ≠ 0, 1, 2, 3, 4

Використовуючи обмеження |x[k]| ≤ 1, отримуємо:
|y[0]| = |x[0]| ≤ 1
|y[1]| = 2|x[1]| ≤ 2
|y[2]| = 3|x[0]| + 3|x[2]| ≤ 6
|y[3]| = 2|x[1]| ≤ 2
...
Отже, найбільше можливе значення |y[k]|, таке, що ця максимальна величина досягається при k = 0, становить 1.
